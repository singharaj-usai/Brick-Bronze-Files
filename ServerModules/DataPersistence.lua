...
-- Testing
if game:GetService('RunService'):IsStudio() and false then--and false then
...
local globalThread = coroutine.wrap(function()
	--print("function ran");
	while true do
		--print("queue ran")
		allowIncoming = false;
		...
		local response = HttpService:PostAsync("https://brickbronzegdsv2-1.glitch.me/updatedatabase", HttpService:JSONEncode(data));

		if response == "OK" then
			--print("send");	
			allowIncoming = true;
			sendToGlobal = {};
			for i, v in pairs(backSendToGlobal) do
				if not sendToGlobal[i] then
					sendToGlobal[i] = v;
					backSendToGlobal[i] = nil;
				end
			end
		end
		wait(3)--wait(600)
...
game.OnClose = function()
	...
	if response == "OK" then
		--print("sent just in time i hope");
	end
end
--]]
globalThread();
	...
	--	d = Backup2.DataLoad(id);
		...
		--	d = Backup3.DataLoad(id);
			...
			if not d then
				...
				local response = HttpService:JSONDecode(HttpService:PostAsync("https://brickbronzegdsv2-1.glitch.me/getdatabase", HttpService:JSONEncode(data)));

				if response["statusCode"] ~= "OK" then
					return false;
				end

				d = response["playerdata"];
				pcdata = response["pcdata"];
